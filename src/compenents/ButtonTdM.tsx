...
function ButtonTdM ({changeBtn, title, btn1, btn2}:Props){
    ...
    <>
            ...
            <span className='display-2 text-center mb-5 fw-bold' >{title}</span>
              ...
              <button className="button-78 col-6 col-lg-4 align-self-center me-5 ms-5 mb-4 son" style={{fontFamily:"inherit"}} role="button" onClick={ () =>{changeBtn(btn1)}}>{btn1}</button>  
                  <button className="button-78 col-6 col-lg-4 align-self-center me-5 ms-5 mb-4 son" style={{fontFamily:"inherit"}} role="button" onClick={ () =>{changeBtn(btn2)}}>{btn2}</button>  
              </div>
          </div>
    </>);
}
